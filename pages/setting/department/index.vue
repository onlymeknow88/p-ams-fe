<template>
  <div>
    <div v-if="departments.result.role.access == 'N'">
              ...
              <div class="card-body">
                <div class="card-title">
                  <div class="col-12 item-title">
                    ...
                    <NuxtLink
                      :to="{ name: 'setting-department-create' }"
                      class="btn btn-icon"
                      title="Add"
                      :class="
                        departments.result.role.create == 'Y' ? '' : 'disabled'
                      "
                    >
                      <svg
                        width="24"
                ...
                <div class="table-responsive">
                  ...
                  <b-table
                    v-else
                    hover
                    :items="departments.result.departments.data"
                    :fields="fields"
                    show-empty
                    :sort-by.sync="sortBy"
                    ...
                    <template v-slot:cell(actions)="row">
                      <div class="d-flex justify-content-start">
                        <NuxtLink
                          ...
                          title="Edit"
                          :to="{
                            name: 'setting-department-edit-id',
                            params: { id: row.item.id },
                          }"
                          :class="
                            departments.result.role.edit == 'Y'
                              ? ''
                              : 'disabled'
                          "
                        >
                        ...
                        <button
                          ...
                          :class="
                            departments.result.role.delete == 'Y'
                              ? ''
                              : 'disabled'
                          "
                        >
                ...
                <div class="col-xl-12 mt-4">
                  <div class="pagination-footer">
                    <span class="form-text-13 mb-3"
                      >Page
                      {{
                        departments.result.departments.from
                          ? departments.result.departments.from
                          : 0
                      }}
                      of
                      {{
                        departments.result.departments.to
                          ? departments.result.departments.to
                          : 0
                      }}
                      Showing
                      {{ departments.result.departments.total }}
                    </span>
                    <b-pagination
                      v-model="page"
...
export default {
  ...
  computed: {
    ...
    errorPage() {
      if (this.departments.result.role.access == 'N') {
  ...
  methods: {
    ...mapActions('setting/department', [
      'getDepartmentsData',
      'destroyDepartmentsData',
    ]),
    ...mapMutations('setting/department', ['SET_PAGE', 'SET_LIMIT']),
    findSearch() {
      clearTimeout(this.timeout)
      this.timeout = setTimeout(async () => {
        // this.search = e.target.value
        this.SET_PAGE(1)
        this.getDepartmentsData(this.search)
      }, 600)
    },
    SortBy(e) {
    ...
    deleteDepartment(id) {
      Swal.fire({
        ...
        if (e.value == true) {
          this.destroyDepartmentsData(id).then()
        }
      })
    },
  },
