              ...
              <div class="col-xl-12 mt-4">
                <div class="pagination-footer">
                  <span class="form-text-13 mb-3"
                    >Page {{ roles.result.from ? roles.result.from : 0 }} of
                    {{ roles.result.to ? roles.result.to : 0 }}
                    Showing
                    {{ roles.result.total }}
                  </span>
                  <b-pagination
                    v-model="page"
                    :total-rows="roles.result.total"
...
export default {
  ...
  async asyncData({ store }) {
    await Promise.all([store.dispatch('setting/role/getRolesData')])
    return
  },
  data() {
    return {
  ...
  methods: {
    ...mapActions('setting/role', ['getRolesData', 'destroyRolesData']),
    ...mapMutations('setting/role', ['SET_PAGE', 'SET_LIMIT']),
    findSearch(e) {
