<template>
  <div>
    <div v-if="manufactures.result.role.access == 'N'">
              ...
              <div class="card-body">
                <div class="card-title">
                  <div class="col-12 item-title">
                    ...
                    <NuxtLink
                      :to="{ name: 'setting-manufacture-create' }"
                      class="btn btn-icon"
                      title="Add"
                      :class="
                        manufactures.result.role.create == 'Y' ? '' : 'disabled'
                      "
                    >
                      <svg
                        width="24"
                ...
                <div class="table-responsive">
                  ...
                  <b-table
                    v-else
                    hover
                    :items="manufactures.result.manufactures.data"
                    :fields="fields"
                    show-empty
                    :sort-by.sync="sortBy"
                  ...
                  >
                    <template v-slot:cell(image)="row">
                      <img
                        :class="{ 'd-none': !row.item.image }"
                        :src="
                          URLbackend + '/storage/manufactures/' + row.item.image
                        "
                        alt=""
                        height="80"
                        width="80"
                    ...
                    <template v-slot:cell(actions)="row">
                      <div class="d-flex justify-content-start">
                        <NuxtLink
                          ...
                          title="Edit"
                          :to="{
                            name: 'setting-manufacture-edit-id',
                            params: { id: row.item.id },
                          }"
                          :class="
                            manufactures.result.role.edit == 'Y'
                              ? ''
                              : 'disabled'
                          "
                        >
                        ...
                        <button
                          ...
                          :class="
                            manufactures.result.role.delete == 'Y'
                              ? ''
                              : 'disabled'
                          "
                        >
                ...
                <div class="col-xl-12 mt-4">
                  <div class="pagination-footer">
                    <span class="form-text-13 mb-3"
                      >Page
                      {{
                        manufactures.result.manufactures.from
                          ? manufactures.result.manufactures.from
                          : 0
                      }}
                      of
                      {{
                        manufactures.result.manufactures.to
                          ? manufactures.result.manufactures.to
                          : 0
                      }}
                      Showing
                      {{ manufactures.result.manufactures.total }}
                    </span>
                    <b-pagination
                      v-model="page"
...
export default {
  ...
  computed: {
    ...
    errorPage() {
      if (this.models.result.role.access == 'N') {
  ...
  methods: {
    ...mapActions('setting/manufacture', [
      'getManufacturesData',
      'destroyManufacturesData',
    ]),
    ...mapMutations('setting/manufacture', ['SET_PAGE', 'SET_LIMIT']),
    findSearch() {
      clearTimeout(this.timeout)
      this.timeout = setTimeout(async () => {
        // this.search = e.target.value
        this.SET_PAGE(1)
        this.getManufacturesData(this.search)
      }, 600)
    },
    SortBy(e) {
    ...
    deleteCompany(id) {
      Swal.fire({
        ...
        if (e.value == true) {
          this.destroyManufacturesData(id).then()
        }
      })
    },
    // sort: function (s) {
  ...
  async fetch() {
    await this.$axios.$get('manufacture')
  },
}
</script>
