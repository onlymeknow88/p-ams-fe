<template>
  <div>
    <div v-if="locations.result.role.access == 'N'">
              ...
              <div class="card-body">
                <div class="card-title">
                  <div class="col-12 item-title">
                    ...
                    <NuxtLink
                      :to="{ name: 'setting-location-create' }"
                      class="btn btn-icon"
                      title="Add"
                      :class="
                        locations.result.role.create == 'Y' ? '' : 'disabled'
                      "
                    >
                      <svg
                        width="24"
                ...
                <div class="table-responsive">
                  ...
                  <b-table
                    v-else
                    hover
                    :items="locations.result.locations.data"
                    :fields="fields"
                    show-empty
                    :sort-by.sync="sortBy"
                    ...
                    <template v-slot:cell(actions)="row">
                      <div class="d-flex justify-content-start">
                        <NuxtLink
                          ...
                          title="Edit"
                          :to="{
                            name: 'setting-location-edit-id',
                            params: { id: row.item.id },
                          }"
                          :class="
                            locations.result.role.edit == 'Y' ? '' : 'disabled'
                          "
                        >
                          <svg
                            width="20"
                        ...
                        <button
                          ...
                          class="btn btn-icon color-softgray-5 mr-6"
                          title="Delete"
                          @click="deleteLocation(row.item.id)"
                          :class="
                            locations.result.role.delete == 'Y'
                              ? ''
                              : 'disabled'
                          "
                        >
                ...
                <div class="col-xl-12 mt-4">
                  <div class="pagination-footer">
                    <span class="form-text-13 mb-3"
                      >Page
                      {{
                        locations.result.locations.from
                          ? locations.result.locations.from
                          : 0
                      }}
                      of
                      {{
                        locations.result.locations.to
                          ? locations.result.locations.to
                          : 0
                      }}
                      Showing
                      {{ locations.result.locations.total }}
                    </span>
                    <b-pagination
                      v-model="page"
...
export default {
  ...
  async asyncData({ store }) {
    await Promise.all([store.dispatch('setting/location/getLocationsData')])
    return
  },
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'location_tag', sortable: false },
  ...
  methods: {
    ...mapActions('setting/location', [
      'getLocationsData',
      'destroyLocationsData',
    ]),
    ...mapMutations('setting/location', [
      'SET_PAGE',
      'SET_LIMIT',
      'SET_NO_DATA',
    ]),
    findSearch() {
      ...
      this.timeout = setTimeout(async () => {
        ...
        this.SET_PAGE(1)
        this.getLocationsData(this.search)
      }, 600)
    },
    SortBy(e) {
    ...
    deleteLocation(id) {
      Swal.fire({
        ...
        if (e.value == true) {
          this.destroyLocationsData(id).then()
        }
      })
    },
    // sort: function (s) {
