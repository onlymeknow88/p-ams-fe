            ...
            <div class="card-body">
              ...
              <div class="table-responsive">
                ...
                <b-table
                  ...
                  label-sort-clear=""
                >
                  <template v-slot:cell(master_menu)="row">
                    {{ row.item.parent ? row.item.parent['nama_menu'] : '-' }}
                  </template>
                  <template v-slot:cell(level_menu)="row">
                    {{
                      row.item.level_menu == 'main_menu'
                        ? 'Main Menu'
                        : 'Sub Menu'
                    }}
                  </template>
                  <template v-slot:cell(icon)="row">
                    <img :src="URLbackend + '/assets/icon/black/' + row.item.icon" alt="">
                  </template>
              ...
              <div class="col-xl-12 mt-4">
                <div class="pagination-footer">
                  <span class="form-text-13 mb-3"
                    >Page {{ menus.result.from ? menus.result.from : 0 }} of
                    {{ menus.result.to ? menus.result.to : 0 }} Showing
                    {{ menus.result.total }}
                  </span>
                  <b-pagination
                    v-model="page"
                    :total-rows="menus.result.total"
...
export default {
  ...
  async asyncData({ store }) {
    await Promise.all([store.dispatch('setting/menu/getMenusData')])
    return
  },
  async fetch() {
  ...
  data() {
    return {
      ...
      fields: [
        { key: 'no_urut', sortable: false },
        { key: 'nama_menu', sortable: true },
        { key: 'master_menu', sortable: false },
        { key: 'level_menu', sortable: false },
  ...
  methods: {
    ...mapActions('setting/menu', ['getMenusData', 'destroyMenusData']),
    ...mapMutations('setting/menu', ['SET_PAGE', 'SET_LIMIT']),
    findSearch() {
