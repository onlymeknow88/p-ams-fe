<template>
  <select class="form-select" name="company_id" id="company_id"></select>
</template>
<script>
export default {
  props: ['edit'],
    ...
    if (this.$route.name == 'assets-list-assets-edit-id') {
      $('#company_id')
        ...
        .on('change', function () {
          ...
          $('#company_id').append(option).change()
    } else {
      $('#company_id')
        .select2({
          theme: 'bootstrap-5',
  ...
  watch: {
    ...
    options: function (options) {
      // update options
      $('#company_id').empty().select2({ data: options })
    },
  },
  destroyed: function () {
    $('#company_id').off().select2('destroy')
  },
}
</script>
