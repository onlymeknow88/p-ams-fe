<template>
  <div>
    <div v-if="categories.result.role.access == 'N'">
              ...
              <div class="card-body">
                <div class="card-title">
                  <div class="col-12 item-title">
                    ...
                    <NuxtLink
                      :to="{ name: 'setting-category-create' }"
                      class="btn btn-icon"
                      title="Add"
                      :class="
                        categories.result.role.create == 'Y' ? '' : 'disabled'
                      "
                    >
                      <svg
                        width="24"
                ...
                <div class="table-responsive">
                  ...
                  <b-table
                    v-else
                    hover
                    :items="categories.result.categorys.data"
                    :fields="fields"
                    show-empty
                    :sort-by.sync="sortBy"
                  ...
                  >
                    <template v-slot:cell(image)="row">
                      <img
                        ...
                        :src="
                          URLbackend + '/storage/categories/' + row.item.image
                        "
                        alt=""
                        height="80"
                        width="80"
                    ...
                    <template v-slot:cell(actions)="row">
                      <div class="d-flex justify-content-start">
                        <NuxtLink
                          ...
                          title="Edit"
                          :class="
                            categories.result.role.edit == 'Y' ? '' : 'disabled'
                          "
                          :to="{
                            name: 'setting-category-edit-id',
                        ...
                        <button
                          ...
                          :class="
                            categories.result.role.delete == 'Y'
                              ? ''
                              : 'disabled'
                          "
                        >
                ...
                <div class="col-xl-12 mt-4">
                  <div class="pagination-footer">
                    <span class="form-text-13 mb-3"
                      >Page
                      {{
                        categories.result.categorys.from
                          ? categories.result.categorys.from
                          : 0
                      }}
                      of
                      {{
                        categories.result.categorys.to
                          ? categories.result.categorys.to
                          : 0
                      }}
                      Showing
                      {{ categories.result.categorys.total }}
                    </span>
                    <b-pagination
                      v-model="page"
...
export default {
  ...
  computed: {
    ...
    errorPage() {
      if (this.categories.result.role.access == 'N') {
  ...
  methods: {
    ...
    ...mapMutations('setting/category', ['SET_PAGE', 'SET_LIMIT']),
    SortBy(e) {
    ...
    deleteCategory(id) {
      Swal.fire({
        ...
        if (e.value == true) {
          this.destroyCategoriesData(id).then()
        }
      })
    },
  },
